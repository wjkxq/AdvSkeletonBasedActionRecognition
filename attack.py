...
class NTU_ADMM_ATTACK:
    ...
    def bone_constraints(self, data_adv, bone_length, epsilon=0.01, mask=None):
        ...
        return (mask_bone * (torch.abs(torch.norm(relative_pos_adv, dim=1) - bone_length)) / (bone_length + 1e-5))
    ...
    def temporal_smoothness_constraints(self, data_adv, velocity, epsilon=0.1, mask=None):
        ...
        return ((mask_vel * torch.norm(velocity_adv - velocity, dim=1) / (torch.norm(velocity, dim=1) + 1e-4)))
